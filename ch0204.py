...
# 输入是一个n维向量x, 输出是一个标量, 函数f(x)相对于x的梯度是一个包含n个偏导数的向量(准确说是列向量),通常表示为▽f(x)或者grad f(x)，
# 这个向量的方向是f(x)在该点增长最快的方向，大小是增长率的最大值
#                       ▽f(x) = [∂f(x)/∂x1, ∂f(x)/∂x2,..., ∂f(x)/∂xn]T
# 
# 
# 梯度的四大性质
#  1. ▽(Ax) = AT: 线性变换的梯度是变换矩阵的转置
#  2. ▽(xTA) = A
#  3. ▽(xTAx) = (A+AT)x 
#  4. ▽||x||^2 = ▽(xTx) = 2x: x的二范数的平方的梯度是2x
